...
export default function HistoryFrontend() {
  ...
  // API থেকে সর্বশেষ History ডাটা নিয়ে আসা
  useEffect(() => {
    const fetchHistoryData = async () => {
      try {
        const res = await fetch('/api/about/history', { cache: 'no-store' });
        const result = await res.json();
        
        // যদি ডাটা থাকে, তবে সর্বশেষ ক্রিয়েট করা কন্টেন্টটি দেখাবে
  ...
  if (loading) {
    return (
      <div className="w-full bg-[#eaf4ed] py-24 flex justify-center items-center">
        <div className="animate-pulse text-[#00873E] font-medium">Loading History...</div>
      </div>
    );
  }
